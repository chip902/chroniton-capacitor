...
async def debug_oauth_states():
    """Debug endpoint to see OAuth states"""
    return {
        "active_states": len(_oauth_states),
        "states": {k: {"provider": v.provider, "redirect_url": v.redirect_url, "created_at": v.created_at} for k, v in _oauth_states.items()}
    }
...
def store_oauth_state(state: str, oauth_state: OAuthState) -> None:
    ...
    print(f"OAUTH DEBUG: Stored OAuth state for {state}: redirect_url={oauth_state.redirect_url}")
    logger.error(f"OAUTH DEBUG: Stored OAuth state for {state}: redirect_url={oauth_state.redirect_url}")
    ...
    print(f"OAUTH DEBUG: OAuth state storage: {len(_oauth_states)} active states")
    logger.error(f"OAUTH DEBUG: OAuth state storage: {len(_oauth_states)} active states")
...
async def google_callback(
    ...
    try:
        ...
        print(f"OAUTH DEBUG: Looking up OAuth state for: {state}")
        logger.error(f"OAUTH DEBUG: Looking up OAuth state for: {state}")
        oauth_state = get_oauth_state(state)
        print(f"OAUTH DEBUG: Found OAuth state: {oauth_state}")
        logger.error(f"OAUTH DEBUG: Found OAuth state: {oauth_state}")
        
        if not oauth_state or oauth_state.provider != "google":
            print(f"OAUTH DEBUG: Invalid OAuth state - oauth_state: {oauth_state}, provider: {oauth_state.provider if oauth_state else 'None'}")
            logger.error(f"OAUTH DEBUG: Invalid OAuth state - oauth_state: {oauth_state}, provider: {oauth_state.provider if oauth_state else 'None'}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        print(f"OAUTH DEBUG: Using redirect_url: {redirect_url}")
        logger.error(f"OAUTH DEBUG: Using redirect_url: {redirect_url}")
        ...
        print(f"OAUTH DEBUG: Checking redirect_url: {redirect_url}")
        logger.error(f"OAUTH DEBUG: Checking redirect_url: {redirect_url}")
        if redirect_url:
            ...
            print("OAUTH DEBUG: No redirect_url found, returning JSON response")
            logger.error("OAUTH DEBUG: No redirect_url found, returning JSON response")
