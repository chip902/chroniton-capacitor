    ...
    def create_event(self, calendar_url: str, event_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.status_code in [201, 204]:
                logger.info(f"Successfully created CalDAV event: {event_data['title']}")
                return True
            else:
                logger.error(f"Failed to create CalDAV event: {response.status_code} - {response.text}")
                return False
    ...
    def update_event(self, event_url: str, event_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                logger.info(f"Successfully updated CalDAV event: {event_data['title']}")
                return True
            else:
                logger.error(f"Failed to update CalDAV event: {response.status_code} - {response.text}")
                return False
    ...
    def delete_event(self, event_url: str) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                logger.error(f"Failed to delete CalDAV event: {response.status_code} - {response.text}")
                return False
