...
class CalendarSyncController:
    ...
    async def push_calendar_metadata_changes(self, changes: Dict[str, Any], target_agents: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if target_agents:
                agents_to_update = [agent for agent in config.agents if agent.id in target_agents]
            else:
                agents_to_update = [agent for agent in config.agents if agent.enabled]
            ...
            for agent in agents_to_update:
                try:
                    ...
                    logger.info(f"Queued calendar metadata update for agent {agent.id}")
        ...
        try:
            ...
            for agent in agents_to_update:
                try:
                    ...
                    await self.storage.queue_agent_update(agent.id, update_data)
                    results["updates_queued"] += 1
                    logger.info(f"Queued sync config update for agent {agent.id}")
    ...
    async def get_pending_updates_for_agent(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error getting pending updates for agent {agent_id}: {e}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    async def mark_agent_update_processed(self, agent_id: str, update_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Marked update {update_id} as processed for agent {agent_id}")
            ...
            logger.error(f"Error marking update as processed for agent {agent_id}: {e}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
